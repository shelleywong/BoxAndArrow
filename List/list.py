...
# file: List/list.py
#
# Flask application for linked list visualization

from flask import Flask, render_template, request, redirect
from flask import make_response, url_for
from forms import ListCmds
import sys
...
@app.route('/')
def index():
    return 'Index for Linked List examples'
...
def list():
    ...
    if request.method == 'GET':
        ...
        print('Launching Linked List Visualization')
        ...
        submit = True
        context = dict(form=form, submit=submit)
        resp = make_response(render_template('list.html', **context))
        return resp
    else:
        context = dict(form=form, submit=submit)
        resp = make_response(render_template('launch.html', **context))
